...
import { getUserPerformance, type PerformanceData } from '@/services/performanceService';
import { 
  GraduationCap, 
  Users, 
  BookOpen, 
...
export default function PermissionBasedDashboard({ userPermissions, userRole }: PermissionBasedDashboardProps) {
  ...
  useEffect(() => {
    const fetchPerformanceData = async () => {
      if (user?.id) {
        try {
          const data = await getUserPerformance(user.id);
    ...
    <div className="space-y-6">
      ...
      {uniqueWidgets.length > 0 && (
        <motion.div
          ...
          className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 pt-6"
        >
          <Shield className="w-4 h-4" />
          <span>You have access to <strong className="text-blue-600 dark:text-blue-400">{uniqueWidgets.length}</strong> module{uniqueWidgets.length !== 1 ? 's' : ''}</span>
        </motion.div>
      )}
          ...
          {uniqueWidgets.map((widget, index) => {
            ...
            return (
              ...
              >
                <Link
                  href={config.href}
                  className="block group bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-100 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 transition-all duration-300 relative overflow-hidden"
