...
export default function IPRModule({ permissions }: IPRModuleProps) {
  ...
  const handleSubmitItem = async (itemId: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        alert('IPR item submitted successfully!');
      }
    } catch (error) {
      console.error('Error submitting IPR item:', error);
      alert('Failed to submit IPR item. Please try again.');
    } finally {
  ...
  const handleReviewItem = async (itemId: string, action: 'approve' | 'reject', comments?: string) => {
    try {
      ...
      if (response.data.success) {
        await fetchIPRData(); // Refresh data
        alert(`IPR item ${action}d successfully!`);
      }
    } catch (error) {
      console.error('Error reviewing IPR item:', error);
      alert(`Failed to ${action} IPR item. Please try again.`);
    }
  };
