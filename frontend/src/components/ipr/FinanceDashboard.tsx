...
import { financeService } from '@/services/ipr.service';
import { 
  DollarSign, 
  Clock, 
  CheckCircle, 
...
export default function FinanceDashboard() {
  ...
  const fetchPendingReviews = async () => {
    try {
      ...
      console.error('Error fetching pending finance reviews:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitReview = async () => {
    ...
    if (formData.auditStatus === 'approved') {
      if (!formData.incentiveAmount || !formData.pointsAwarded) {
        alert('Incentive amount and points are required for approval');
        return;
      }
    } else {
    ...
    try {
      ...
      if (formData.auditStatus === 'approved') {
        ...
        alert('Incentive processed successfully!');
      } else {
        // Reject
        await financeService.submitReview(selectedApp.id, {
