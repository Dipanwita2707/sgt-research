...
import { drdReviewService } from '@/services/ipr.service';
...
export default function DrdReviewDashboard() {
  ...
  const fetchUserPermissions = async () => {
    try {
      ...
      console.log('DrdReviewDashboard - User permissions:', allPermissions);
  ...
  const fetchPendingReviews = async () => {
    try {
      ...
      setError(error.response?.data?.message || 'Failed to fetch pending reviews. Please check your permissions.');
      setApplications([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleReviewClick = async (app: any) => {
    try {
      ...
      alert(error.response?.data?.message || 'Failed to fetch application details. Please check your permissions.');
    }
  };
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (reviewData.decision === 'approved') {
        ...
        if (!canApprove) {
          alert('You do not have permission to approve. Please use "Recommend" instead.');
          return;
        }
        await drdReviewService.approveReview(selectedApp.id, reviewData.comments);
        alert('Application approved successfully! Incentive points have been credited.');
        ...
        if (!reviewData.comments.trim()) {
          alert('Comments are required when recommending');
          return;
        }
        await drdReviewService.recommendReview(selectedApp.id, reviewData.comments);
        alert('Application recommended successfully! It will now go to DRD Head for final approval.');
        ...
        if (!reviewData.comments.trim()) {
          alert('Comments are required when requesting changes');
          return;
        }
  ...
  const handleQuickApprove = async (app: any) => {
    if (!confirm(`Are you sure you want to approve "${app.title}"? This will calculate and credit incentive points.`)) {
      return;
    }

    try {
      const result = await drdReviewService.approveReview(app.id, 'Quick approval by DRD');
      alert(`Application approved successfully! Points awarded: ${result.pointsAwarded || 0}, Amount: ₹${result.incentiveAmount || 0}`);
      fetchPendingReviews();
    } catch (error: any) {
  ...
  const handleRecommendToHead = async (app: any) => {
    ...
    try {
      ...
      alert('Application recommended to DRD Head successfully!');
      fetchPendingReviews();
    } catch (error: any) {
      console.error('Recommend to head error:', error);
      alert(error.response?.data?.message || 'Failed to recommend application');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleHeadApprove = async (app: any) => {
    if (!confirm(`Are you sure you want to approve "${app.title}" and submit to Government?`)) return;
    ...
    try {
      ...
      if (response.data.success) {
        alert('Application approved and marked as submitted to Government!');
        fetchPendingReviews();
      } else {
  ...
  const handleAddGovtId = async () => {
    if (!selectedApp || !govtApplicationId.trim()) {
      alert('Please enter Government Application ID');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        alert('Government Application ID added successfully!');
        setShowGovtIdModal(false);
        setGovtApplicationId('');
        setSelectedApp(null);
        fetchPendingReviews();
      } else {
        alert(response.data.message || 'Failed to add Government Application ID');
      }
    } catch (error: any) {
      console.error('Add govt ID error:', error);
      alert(error.response?.data?.message || 'Failed to add Government Application ID');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleAddPublicationId = async () => {
    if (!selectedApp || !publicationId.trim()) {
      alert('Please enter Publication ID');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        alert('Publication ID added successfully! IPR marked as Published.');
        setShowPubIdModal(false);
        setPublicationId('');
        setSelectedApp(null);
        fetchPendingReviews();
      } else {
        alert(response.data.message || 'Failed to add Publication ID');
      }
    } catch (error: any) {
      console.error('Add publication ID error:', error);
      alert(error.response?.data?.message || 'Failed to add Publication ID');
    } finally {
      setSubmitting(false);
    }
  };
                ...
                {(selectedApp.annexureFilePath || (selectedApp.supportingDocsFilePaths && selectedApp.supportingDocsFilePaths.length > 0)) ? (
                  <div className="space-y-3">
                    ...
                    {selectedApp.annexureFilePath && (() => {
                      ...
                      const fileUrl = filePath.startsWith('http') ? filePath : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/${filePath}`;
