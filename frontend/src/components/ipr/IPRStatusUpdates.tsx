...
import { iprService, IprStatusUpdate } from '@/services/ipr.service';
...
export default function IPRStatusUpdates({ applicationId, isDRD = false, timelineMode = false }: IPRStatusUpdatesProps) {
  ...
  const handleAddUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setError('');
      const response = await fetch(`/api/v1/drd-review/status-update/${applicationId}`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to add status update');
      }
  ...
  const handleDeleteUpdate = async (updateId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/drd-review/status-update/${updateId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
