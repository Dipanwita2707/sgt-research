...
export default function MentorCollaborativeReviewModal({ 
  ...
  const loadEditSuggestions = async () => {
    try {
      ...
      console.error('Failed to load mentor edit suggestions:', error);
    }
  };
  ...
  const handleMentorReview = async () => {
    ...
    try {
      if (reviewData.decision === 'approved') {
        ...
        alert('Application approved successfully! It will now be submitted to DRD for review.');
        ...
        if (pendingSuggestionsCount > 0) {
          alert(`Changes requested successfully! The student will see ${pendingSuggestionsCount} edit suggestion(s) to review.`);
        } else {
          alert('Changes requested successfully! The student will receive your feedback.');
        }
      }
