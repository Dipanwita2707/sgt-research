...
import collaborativeEditingService, { EditSuggestion } from '@/services/collaborativeEditing.service';
...
export default function CollaborativeReviewModal({ 
  ...
  const loadEditSuggestions = async () => {
    try {
      ...
      console.error('Failed to load edit suggestions:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCollaborativeReview = async () => {
    if (pendingSuggestionsCount > 0) {
      const confirmSubmit = confirm(
        `There are ${pendingSuggestionsCount} pending suggestions. Do you want to submit the review with pending suggestions? The applicant will need to respond to them.`
      );
    ...
    try {
      ...
      if (reviewData.decision === 'approved') {
        ...
        alert('Application approved successfully! Any pending suggestions have been finalized.');
        ...
        if (pendingSuggestionsCount === 0 && !reviewData.comments.trim()) {
          alert('Please create edit suggestions or provide comments when requesting changes');
          return;
        }
        ...
        alert(`Changes requested successfully! The applicant will see ${pendingSuggestionsCount} edit suggestions to review.`);
      }
