...
export default function GoogleDocsEditor({
          ...
          <div key={change.id} className="relative inline">
            ...
            <span className="inline-flex items-center ml-2">
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                ...
                {change.status === 'accepted' && <Check className="w-3 h-3" />}
                {change.status === 'rejected' && <X className="w-3 h-3" />}
              </span>
            </span>
          </div>
        );
  ...
  const hasPendingReviewerChanges = changes.some(c => c.status === 'pending' && c.reviewer !== 'Applicant');
  
  // Check if there are pending changes from applicant
  const hasPendingApplicantChanges = changes.some(c => c.status === 'pending' && c.reviewer === 'Applicant');
