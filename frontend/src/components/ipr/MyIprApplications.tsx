...
export default function MyIprApplications() {
  ...
  const handleSelectAll = (appId: string, action: 'accept' | 'reject') => {
    ...
    if (skippedCount > 0) {
      alert(`${skippedCount} suggestion(s) with invalid values were skipped. Invalid values cannot be accepted - please reject them manually.`);
    }
  };
  ...
  const handleSubmitBatchResponses = async (appId: string) => {
    ...
    if (selectedCount === 0) {
      alert('Please select at least one suggestion to respond to.');
      return;
    }
    ...
    if (!allResponded) {
      const proceed = confirm(
        `You have selected ${selectedCount} out of ${pendingSuggestions.length} suggestions. ` +
        `Do you want to submit responses for the selected ones only?`
      );
      if (!proceed) return;
    }
  ...
  const getApplicationCardStyle = (app: any, pendingSuggestions: EditSuggestion[]) => {
    ...
    if (app.status === 'completed' || app.status === 'published') {
      return 'border-green-300 bg-gradient-to-r from-green-50 to-white';
    }
    if (app.status === 'drd_rejected') {
  ...
  const getApplicationsToDisplay = () => {
    switch (activeTab) {
      case 'action_required':
        return applications.filter(app => app.status === 'changes_required' || app.status === 'draft');
      case 'in_progress':
        return applications.filter(app => 
          ['submitted', 'under_drd_review', 'recommended_to_head', 'resubmitted', 'drd_head_approved', 'submitted_to_govt', 'govt_application_filed'].includes(app.status)
        );
      case 'completed':
        return applications.filter(app => 
          ['completed', 'published', 'drd_rejected', 'cancelled'].includes(app.status)
        );
      case 'draft':
  ...
  const getPendingSuggestions = (appId: string) => {
    return (suggestions[appId] || []).filter(s => s.status === 'pending');
  };
