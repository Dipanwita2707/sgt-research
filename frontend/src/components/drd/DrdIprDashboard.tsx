...
export default function DrdIprDashboard() {
  ...
  const fetchUserPermissions = async () => {
    try {
      ...
      if (response.data.success && response.data.data.permissions) {
        console.log('Full permissions response:', response.data.data.permissions);
        
        response.data.data.permissions.forEach((dept: any) => {
          console.log('Processing department:', dept.category, 'with permissions:', dept.permissions);
      ...
      console.log('Extracted ALL permissions:', allPermissions);
  ...
  const handleSubmitAction = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.message || `Failed to process ${actionType}`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  console.log('Permission checks (simplified):', {
