...
import { centralDepartmentService, CentralDepartment } from '@/services/centralDepartment.service';
...
export default function EmployeeManagement() {
  ...
  const fetchSchools = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/v1/schools', {
        withCredentials: true,
      });
  ...
  const fetchCentralDepartments = async () => {
    try {
      ...
      console.error('Error fetching central departments:', error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingEmployee) {
        await axios.put(
          `http://localhost:5000/api/v1/employees/${editingEmployee.id}`,
          finalFormData,
          { withCredentials: true }
        );
        alert('Employee updated successfully!');
      } else {
        await axios.post('http://localhost:5000/api/v1/employees', finalFormData, {
          withCredentials: true,
        });
  ...
  const handleToggleStatus = async (employee: Employee) => {
    try {
      await axios.patch(
        `http://localhost:5000/api/v1/employees/${employee.id}/toggle-status`,
        {},
        { withCredentials: true }
      );
      fetchEmployees();
  ...
  }, [searchQuery, filterRole, filterCategory]);
