...
export default function GrantReviewDashboard() {
  ...
  const fetchGrants = async () => {
    try {
      ...
      setError(error.response?.data?.message || 'Failed to fetch grant applications');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRecommend = async (grantId: string) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Failed to recommend grant');
    }
  };
  ...
  const handleApprove = async (grantId: string) => {
    if (!confirm('Are you sure you want to approve this grant application?')) return;
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Failed to approve grant');
    }
  };
  ...
  const handleReject = async (grantId: string) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Failed to reject grant');
    }
  };

  const handleMarkCompleted = async (grantId: string) => {
    if (!confirm('Mark this grant as completed?')) return;
