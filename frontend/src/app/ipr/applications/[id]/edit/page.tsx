...
export default function EditApplicationPage({ params }: EditApplicationPageProps) {
  ...
  const API_BASE = (process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5000').replace(/\/$/, '');
  const API_WITH_V1 = API_BASE.includes('/api/v1') ? API_BASE : `${API_BASE}/api/v1`;
  const HOST_BASE = API_BASE.includes('/api/v1') ? API_BASE.replace('/api/v1', '') : API_BASE;
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            {application.annexureFilePath && (
              <div className="mb-3">
                ...
                <a
                  href={`${HOST_BASE}/uploads/${application.annexureFilePath}`}
                  target="_blank"
            ...
            {application.prototypeFilePath && (
              <div className="mb-3">
                ...
                <a
                  href={`${HOST_BASE}/uploads/${application.prototypeFilePath}`}
                  target="_blank"
                  rel="noopener noreferrer"
