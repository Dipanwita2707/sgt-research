...
export default function GrantReviewPage() {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      if (!grant?.projectCategory || !grant?.projectType) {
        console.log('Cannot fetch policy - missing data:', { 
      ...
      console.log('Fetching policy for:', {
        projectCategory: grant.projectCategory,
        projectType: grant.projectType
      });
      ...
      try {
        ...
        console.log('Fetched grant policy:', policy);
  ...
  const fetchPermissions = async () => {
    try {
      ...
      console.log('DRD Permissions:', drdPermissions);
  ...
  const calculateInvestigatorIncentive = (
    ...
    console.log('calculateInvestigatorIncentive called:', { roleType, investigatorCategory, activePolicy, grant });
    
    // External investigators get ZERO
    if (investigatorCategory !== 'Internal') {
      console.log('Returning 0 - investigator is External');
      return { incentive: 0, points: 0 };
    }

    // No policy loaded yet
    if (!activePolicy || !grant) {
      console.log('Returning 0 - no policy or grant:', { activePolicy: !!activePolicy, grant: !!grant });
      return { incentive: 0, points: 0 };
    }
    ...
    console.log('Base amounts:', { totalIncentive, totalPoints });
    ...
    const internalTeamMembers = grant.investigators?.filter(
      (inv: any) => {
        ...
        console.log('Checking team member:', { 
          name: inv.name, 
    ...
    console.log('Total internal count:', totalInternalCount);
    
    if (totalInternalCount === 0) {
      console.log('Returning 0 - no internal investigators');
      return { incentive: 0, points: 0 };
    }
    ...
    if (activePolicy.splitPolicy === 'equal') {
      ...
      console.log('Equal split result:', { perPersonIncentive, perPersonPoints });
  ...
  const handleRequestChanges = async () => {
    if (!reviewComments.trim() && fieldSuggestions.length === 0) {
      alert('Please provide comments or field suggestions');
      return;
    }
  ...
  const handleApprove = async () => {
    if (!confirm('Approve this grant application?')) return;
  ...
  const handleReject = async () => {
    ...
    if (!confirm('Reject this grant? This cannot be undone.')) return;
  ...
  console.log('Grant Status:', grant.status);
  console.log('Can Review Again:', canReviewAgain);
  console.log('Can Approve After Recommending:', canApproveAfterRecommending);
  console.log('User Has Reviewed Current Submission:', userHasReviewedCurrentSubmission);
        ...
        {grant.proposalFilePath && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <a
                href={`http://localhost:5000${grant.proposalFilePath}`}
                target="_blank"
